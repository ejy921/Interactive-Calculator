 ...
 * @author: Jinny Eo
 * 
 * Contains the evaluate(String exp) function, which takes in an expression as a String 
 * and evaluates it as a BigFraction. Includes error checking for invalid expressions. Contains 
 * the store(char register) function, which saves the last evaluated value into a variable and 
 * array that is declared as a field in the class. The getStoreVal(char ch) helps to retrieve 
 * the saved BigFractions in the fields from either classes in the package.
 */
...
public class BFCalculator {
  ...
  /*
   * Evaluates a string that contains an expression. Handles errors, such as when user types in an invalid expresion
   */
  public BigFraction evaluate(String exp) {
    ...
    for (int a = 0; a < expArr.length; a+= 2) {
      ...
      if (Character.isDigit(expArr[a].charAt(0)) && Character.isDigit(expArr[a].charAt(expArr[a].length() - 1))) {
    ...
    for (int i = 1; i < array.length; i+= 2) {
      operator = expArr[i];
      rightVal = array[i + 1];
      // every time there is a calculation, saves to leftVal so that it becomes the new leftVal to calculate
...
/*
 * Stores the last calculated value to storedArr in a way so that
 * the index aligns with the character's position in the alphabet
 */
